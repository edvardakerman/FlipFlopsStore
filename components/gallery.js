class Gallery extends HTMLElement {
    ...
    render(items) {
        ...
        let itemsToRender = items.map(item => {
            return `
            <product-item
                product-name="${item.name ? item.name : 'Product Name'}"
                product-description="${item.description ? item.description : 'Product description.'}"
                product-price="${item.price ? item.price : 0}"
                image-src="${item.image ? item.image : './public/ocean.jpg'}"
                image-alt="${item.imageAlt ? item.imageAlt : './public/ocean_alt.jpg'}"
            ></product-item>`;
        })
